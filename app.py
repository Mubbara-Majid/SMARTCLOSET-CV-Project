...
with st.sidebar:
    ...
    if st.button("✨  Outfit Stylist (AI)", key="nav_stylist", use_container_width=True,
                 type="primary" if st.session_state.page == 'stylist' else "secondary"):
        st.session_state.page = 'stylist'
        st.rerun()
    
    if st.button("📤  Upload New Item", key="nav_upload", use_container_width=True,
                 type="primary" if st.session_state.page == 'upload' else "secondary"):
        st.session_state.page = 'upload'
        st.rerun()
    
    if st.button("📊  Wear Stats", key="nav_stats", use_container_width=True,
...
if st.session_state.page == 'wardrobe':
    ...
    if len(st.session_state.closet) > 0:
        ...
        st.markdown("""
            <div style="text-align: center; padding: 4rem; background: white; border-radius: 20px; margin-top: 2rem;">
                ...
                <p style="color: #9B9BB8;">Start by uploading some items!</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("""
        <div style="background: white; padding: 3rem; border-radius: 20px; text-align: center;">
            <h2 style="color: #1a1a1a; margin-bottom: 1rem;">📸 Upload Your Wardrobe Items</h2>
            <p style="color: #5B5B8D;">Upload clear photos of your clothing items</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    if len(tops) >= 1 and len(bottoms) >= 1 and len(shoes) >= 1:
        ...
        st.warning("⚠️ Need at least 1 top, 1 bottom, and 1 shoe!")
